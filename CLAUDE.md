...
- **Architecture & Conventions:**
  ...
  - Barra sticky della landing (`site-nav.tsx`, 2026-08-19): le voci in riga compaiono solo da `md` (768px) in su — sotto vanno in overflow orizzontale, perché logo + pill delle tre voci + CTA chiedono ~730px. Sotto `md` le stesse voci vivono in un menu a comparsa (Popover) aperto da un bottone pill con icona a destra; le etichette restano intere (non accorciarle). Il bottone "Crea itinerario" resta `hidden sm:inline-flex`. Se si aggiunge una quarta voce, va nel menu senza modifiche, ma va ricontrollato che a 768px la riga stia ancora dentro.
